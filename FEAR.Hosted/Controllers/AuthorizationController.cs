...
using FEAR.Hosted.Helpers;
using FEAR.Hosted.Pages.Authorization;
...
namespace FEAR.Hosted.Controllers
{
    ...
    public class AuthorizationController : Controller
    {
        ...
        public async Task<IActionResult> Authorize()
        {
            ...
            UserInfo[] userInfo = _identityProvider.GetUserInfoFromClaim(result.Principal);
            ...
            switch (await _applicationManager.GetConsentTypeAsync(application))
            {
                ...
                case ConsentTypes.Explicit when authorizations.Count is not 0 && !request.HasPromptValue(PromptValues.Consent):
                    ...
                    identity.SetClaim(Claims.Subject, user.UserId.ToString())
                            .SetClaim(ClaimTypes.Sid, user.UserId.ToString())
                            .SetClaim(Claims.Email, user.Email)
                            .SetClaim(Claims.Name, user.UserName)
                            .SetClaim(Claims.PreferredUsername, user.UserName)
                            // These will need to be "INV:<ROLE>:ID" or "SYS:<ROLE>"
                            .SetClaims(Claims.Role, user.GetRoles().ToImmutableArray());
        ...
        public async Task<IActionResult> Accept()
        {
            ...
            // Retrieve the profile of the logged in user.
            UserInfo[] userInfo = _identityProvider.GetUserInfoFromClaim(User);
            FEAR.Domain.Dto.Authentication.User user = _identityProvider.GetUserFromClaim(userInfo) ??
                throw new InvalidOperationException("The user details cannot be retrieved.");
        ...
        public async Task<IActionResult> Exchange()
        {
            ...
            if (request.IsAuthorizationCodeGrantType() || request.IsRefreshTokenGrantType())
            {
                ...
                if (!_identityProvider.CanSignIn(user))
                {
                    return Forbid(
                        authenticationSchemes: OpenIddictServerAspNetCoreDefaults.AuthenticationScheme,
                        properties: new AuthenticationProperties(new Dictionary<string, string?>
