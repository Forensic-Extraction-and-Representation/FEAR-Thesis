...
{
    /// <summary>
    /// Represents a remote (e.g., Sparql-backed) materialized knowledge graph in FEAR.
    /// Provides logic for synchronizing between a remote graph and a local working graph.
    /// </summary>
    public class SparqlRemoteGraph : BaseRemoteGraph, IRemoteGraph<SparqlParameterizedString>
    {
        public override string GraphLanguage => "SPARQL";
        private SparqlGraphDBConnectionFactory ConnectionFactory { get; set; }
        private SparqlGraphDBConnection Connection { get; set; }
        ...
        /// </summary>
        public SparqlRemoteGraph(SparqlGraphDBConnectionFactory connectionFactory, FindEntityStrategyFactoryManager findEntityStrategyFactoryManager,
        ...
        private string BuildDiagnosticMessage(string operationType, SparqlParameterizedString query, Exception ex)
        {
            ...
            sb.AppendLine($"[SparqlRemoteGraph] {operationType} execution failed.");
        ...
        private void SendErrorTelemetry(string diagnosticMessage)
        {
            TelemetryService?.SendSignal(
                new GenericFEARTelemetrySignal(nameof(SparqlRemoteGraph), FEARTelemetrySerializationEnum.Raw, FEARTelemetrySignalTypeEnum.Error)
